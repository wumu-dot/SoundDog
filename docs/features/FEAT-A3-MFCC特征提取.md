...
- **优先级**：P1
- **预估总耗时**：8h
- **当前状态**：🟢已收官（2026-08-30：4 个子FEAT 全 🟢 且各经独立评审；DoD 三项全勾）
...
| FEAT-A3-04 | 特征向量验证 | 串口打印 13 维，与已知信号特征吻合 | P1 | 依赖 03 | 🟢完成（2026-08-30：三态两两差异三重证据 + e 钳位清偿 + 评审 I-1~M-4 全修复） | `FEAT-A3-04-特征向量验证.md` |
...
- [x] 项目表全部子FEAT 🟢 且每个经 Review（A3-01/02/03/04 各经 R23 独立评审）
- [x] 本表状态与 `docs/features/INDEX.md` 一致（收官时同步更新）
- [x] 经验教训已提炼（A3-03 Mel 域归一化口径 + 帧回调反模式 + 坑 8；A3-04 方案 A' 复用口径 + 文档覆写防御流程）
...
| 2026-08-30 | A3-04 子FEAT 收官 + **A3 父FEAT 收官** | Dev/Test/Review | A3-04 🟢（项目表回填）：纯验证 FEAT（零补打印——A3-03 已备）+ e 钳位 1 行清偿（A3-01 Minor#2 闭环）；方案 A' 复用 A3-03 三态人亲测数据（固件差异仅防护性 1 行，评审核实 VCVT 等价）；三态两两差异三重证据（c0 -620/-3480/-860 + c1 符号反转 + MEL b=7 vs b=23）；R23 独立评审 Ready with fixes（I-1 出处口径更正/I-2 第 8 次覆写 restore/I-3/M-1~4 全修）。**A3 父FEAT 🟢**：4 子FEAT 全收官 + DoD 三项全勾。**A4 立项输入汇总**：① 13 维 float32 s_mfcc13 输入可信（三态基线 A3-04 §5.3）② 100 帧/s 规整 10ms 网格（帧回调）③ A4-01 硬约束：模版 10ms 帧移同网格 + 复刻 Mel 域归一化位置 ④ c1 谱形符号可作强区分特征 | 文档覆写累计第 6~9 次（A3-03/A3-04 文档，防御流程即查即恢复） |
