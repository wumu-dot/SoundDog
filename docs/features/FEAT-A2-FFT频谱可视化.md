...
|-----------|------|----------|--------|------|------|------|
| FEAT-A2-01 | CMSIS-DSP 库集成 | 编译通过，`arm_rfft_fast_f32` 示例跑通（无 NaN/溢出） | P0 | 依赖 A1 | 🔴未开始 | `FEAT-A2-01-CMSIS-DSP集成.md` |
| FEAT-A2-02 | 256点 FFT + 幅度谱 | 1kHz 测试信号 → 峰值落在 bin 15~17 | P0 | 依赖 01 | 🔴未开始 | `FEAT-A2-02-FFT幅度谱.md` |
...
- **阻塞**：依赖 A1 完成（音频链路 + SCK 时钟核实）
- **下一步**：A1 通过后开 A2-01 CMSIS-DSP 集成
