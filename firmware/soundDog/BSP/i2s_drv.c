 ...
 *
 * INMP441 数据提取（实测修正 v10，详见 BUG-20260816-002）：
 *   RAW dump 实测 4 字周期：[音频字][低字节+0x7F 伪迹][0xFFFF 悬空][0xFFFF 悬空]。
 *   仅 word[4k] 含真实 PCM（其低 16 位），extract_frame 步进 4 提取，其余丢弃。
 *   512 字/半区 → 128 有效采样 × 125 帧/s = 16 kHz，与设计一致。
 *   I2S_DRV_ExtractPCM 用 `raw & 0xFFFF` 取音频字低 16 位作为 16-bit PCM。
 */
...
{
    /* v10（BUG-002 确诊 2026-08-28）：4 字周期 [音频][低字节+7F][悬空][悬空]，
     * 仅 word[4k] 低 16 位是真实 PCM，步进 4 提取，其余丢弃。
     * 512 字 → 128 个有效采样（125 帧/s × 128 = 16 kHz，与设计一致） */
    uint32_t out = 0;
    for (uint32_t i = 0; i + 3 < count; i += 4) {
        current_frame.pcm[out++] = I2S_DRV_ExtractPCM(src[i]);
    }

    current_frame.samples      = (uint16_t)out;
