 ...
 *
 * INMP441 在 32-bit I2S 槽的高 24-bit 输出数据（MSB 对齐）。
 * 本函数右移 8 位得到 24-bit 有符号值，再取高 16 位转为 16-bit PCM。
 *
...
{
    int32_t pcm24 = (int32_t)(raw >> 8);   /* 24-bit 补码 */
    return (int16_t)(pcm24 >> 8);           /* 取高 16 位 */
}
