...
| 参数               | 输入值           | 计算结果                             |
| ---------------- | ------------- | -------------------------------- |
| I2S Clock Source | **PLLI2S\_R** | 使用专用 I2S PLL                     |
| **PLLI2S\_N**    | **×256**      | VCO = 1MHz × 256 = 256 MHz       |
| **PLLI2S\_R**    | **/5**        | I2SxCLK = 256 / 5 = **51.2 MHz** |
...
| PLLI2S\_N | PLLI2S\_R | I2SxCLK  | I2SDIV | SCK             |
| --------- | --------- | -------- | ------ | --------------- |
...
| -------- | ---- | ----------- | ---------------- |
| I2S3\_CK | PC10 | 搜 `I2S3_CK` | I2S3 Clock       |
| I2S3\_WS | PA4  | 搜 `I2S3_WS` | I2S3 Word Select |
| I2S3\_SD | PC12 | 搜 `I2S3_SD` | I2S3 Serial Data |
...
| 参数          | 值                        | 说明                      |
| ----------- | ------------------------ | ----------------------- |
| DMA Request | **SPI3\_RX**             | I2S3 就是复用 SPI3 的 DMA 通道 |
...
| Data Width → Memory            | **Word** (32-bit) | 缓冲区用 uint32\_t   |
...
| 信号          | 引脚  | Mode             | 初始电平 |
| ----------- | --- | ---------------- | ---- |
| LED\_ALARM  | PB0 | Output Push-Pull | Low  |
| LED\_STATUS | PB1 | Output Push-Pull | Low  |
...
**Pinout & Configuration → Middleware → FREERTOS → Interface: CMSIS\_V2**
