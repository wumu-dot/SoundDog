...
> **项目描述**：SoundDog 是一个 STM32F407ZGT6 音频异常检测项目。完整链路：INMP441 麦克风 + I2S3 + DMA 采集 16kHz 音频 → FreeRTOS 队列 → 256 点 FFT 频谱（OLED 柱状图 + BANDS 串口）→ MFCC 13 维特征（预加重/分帧/Mel/Log/DCT，100 帧/s）→ K-means 16 质心模版欧氏距离 d\_min 实时比对 → 连续帧防抖判决 → RS485 参数可调（在途）。主控 HSE 8MHz → SYSCLK 168MHz，RTOS 为 FreeRTOS CMSIS-RTOS\_v2，工具链 Makefile + arm-none-eabi-gcc。

> 交接日期：2026-08-16（2026-09-01 A4 中期更新）
> 交接原因：切换 AI 助手
> **最重要的一句话（2026-09-01）**：A1/A2/A3 全线收官，A4-01 + A4-02 + **A4-03（连续帧防抖）均已收官**；**A4-04 参数可调正处于阶段 1/2 已完成、阶段 3（param.c 编码）待启动前的停等点**——接手第一件事是读 §零 的快速上手路径，然后从 A4-04 阶段 3 继续。
...
3. **本文档 §零点五**（2min）——在途任务 A4-04 的精确状态与设计决策
4. **`docs/features/维护地图.md`**（2min）——每个源文件/脚本/数据文件的归属 FEAT 与状态
5. **`docs/features/FEAT-A4-04-参数可调.md`**（4min）——在途 FEAT 全文（重点 §1.4 帧协议/参数集、§执行路线图）
6. 开工前自检：`git status`（应符合当前 FEAT 阶段预期）、`git log --oneline -5`（HEAD 应为 A4-03 收官/近端提交）
...
## 零点五、当前在途任务：A4-04 参数可调（阶段 1/2 → 3 停等点）

> 这是接手后唯一需要立即推进的工作。2026-09-01 状态快照（详细设计以 `docs/features/FEAT-A4-04-参数可调.md` 为准）。

* **进度**：阶段 1（依赖确认/影响范围）+ 阶段 2（帧协议设计）✅ 已完成并经人确认；**下一步 = 阶段 3 编码（param.c）**

* **目标**：RS485（USART3 + TTL-RS485，3.3V/SP3485）在线下发异常检测参数——阈值 TH（A4-02 d\_min）+ 触发/解除帧数（A4-03 的 30/60），改参无需重烧。

* **帧协议（§1.4c 人确认，防自造）**：固定 8 字节 `0xA5|0x01|0x21|PID|值LSB|值MSB|CRC高|CRC低`；CRC16=Modbus 标准（poly 0x8005/init 0xFFFF/反射/大端发送，与 libmodbus 表一致，crcmod 交叉验算钉死）。校验向量：`A5 01 21 01 20 03`(TH=8.0)→`13 27`，`A5 01 21 02 28 00`(触发40)→`D2 90`。

* **可调参数集**：0x01 TH(6.0, 1.0\~20.0, ×100定点) / 0x02 触发连续帧(30) / 0x03 解除连续帧(60) / 0x04 滑窗(固定=100拒改, 位图耦合) / 0x05 窗内触发帧(30)。

* **越界/坏帧策略**：越界拒绝保留旧值；坏帧（帧头/地址/命令/CRC错）丢弃+计数。滑窗 ID 拒改防 DB\_BITMAP\_WORDS 越界。

* **阶段 3 待办**：①新建 `BSP/param.c`（逐字节状态机 param\_feed\_byte + param\_apply 范围校验，接口已定稿在 param.h）②改 `debounce.h/c` 宏→运行期变量 + `debounce_set_params()` ③改 `freertos.c`：`debounce_process(s_dmin > DB_THRESHOLD`(L216) 改读运行期值 + 新增 paramTask（收 USART3→解析→生效）④Makefile C\_SOURCES 补 param.c（BUG-002 教训）⑤build.bat + build\_and\_flash.bat。

* **前置已就绪**：USART3 已初始化（PD8/PD9，115200，main.c:242）；`BSP/rs485_drv.{h,c}`（PD11 EN 高发低收）；`scripts/gen_param_frames.py` 生成测试帧。

* **已知限制**：参数不落 Flash，重启恢复默认（A7-03 未实现）。

* **来源影射（R32 硬约束）**：所有引用代码必须附可点击 URL（modbus.org spec / qModMaster libmodbus / rippleone crc16 / Coskunruhi STM32-UART-Parser-CRC16），禁止"据说/广泛验证"无链接声称。详见 FEAT-A4-04 §1.4c「CRC16 来源」。
...
1. **A4-04 阶段 3 编码**（唯一在途，见 §零点五）→ 阶段 4 实测 → 阶段 5 评审收官
2. ~~A4-03 状态防抖~~（已收官；阶段3 编码 + 边界实测 + 评审均完成）
